...
const EnterPayment = ({ close, onCompleted }) => {
   ...
   const createCustomer = (cardToken: string) => {
      setLoading(true);
      apiService.post(
         API_URLS.CREATECUSTOMER,
         {
            userName: localStorageService.getUserSub(),
            cardToken,
         },
         (response) => {
            toast.success(successMessages.STRIPE_SUCCESS);
            let userDetail = localStorageService.getUserDetail();
            userDetail.organization.subscriptionId = response.data?.result?.subscriptionId;
            localStorageService.storeAuthUser(userDetail);
   ...
   return (
      <Container>
         ...
         <BackContainer onClick={() => localStorageService.clearLocalStorage()}>
            <Link to="/login">
               <BackText>
                  <Icon
                     icon={Icons.Back}
