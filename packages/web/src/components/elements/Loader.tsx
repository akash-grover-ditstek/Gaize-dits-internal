...
const StyledLoader: React.FC<IStyleLoader> = styled.div<IStyleLoader>`
   position: relative;
   top: 1.5px;
   .lds-ring {
      display: inline-block;
      position: relative;
      width: ${(props) => scale(props.size || 1, 1)};
      height: ${(props) => scale(props.size || 1, 1)};
   }
   .lds-ring div {
      box-sizing: border-box;
      display: block;
      position: absolute;
      width: ${(props) => scale(props.size || 1, 0.8)};
      height: ${(props) => scale(props.size || 1, 0.8)};
      margin: ${(props) => scale(props.size || 1, 0.1)};
      border: ${(props) => scale(props.size || 1, 0.066)} solid
         ${(props) => props.color};
      border-radius: 50%;
...
export default function Loader({
   ...
   return (
      <StyledLoader size={LoaderSizesMap[size]} color={color}>
